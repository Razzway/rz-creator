...
function Utils:showNotification(msg, hudColorIndex)
...
function Utils:input(TextEntry, ExampleText, MaxStringLenght)
...
function Utils:SetPlayerBuckets(val)
    TriggerServerEvent(_Prefix..":setBuckets", val)
end

function Utils:PlayAnimeCreator()
...
function Utils:IsDateCorrect(date)
...
function Utils:goCloak()
    ...
    Wait(1200)
    SetEntityCoords(PlayerPedId(), _Config.cloakRoom.pos, false, false, false, false)
    SetEntityHeading(PlayerPedId(), _Config.cloakRoom.heading)
    SetEntityInvincible(PlayerPedId(), false)
    FreezeEntityPosition(GetPlayerPed(-1), false)
    DoScreenFadeIn(750)
    _Client.open:creatorMenu()
end

function Utils:goKitchen()
    ClearPedTasks(PlayerPedId())
    SetEntityInvincible(PlayerPedId(), true)
    FreezeEntityPosition(GetPlayerPed(-1), true)
    DoScreenFadeOut(2000)
    while not IsScreenFadedOut() do Wait(1) end
    Wait(1200)
    SetEntityCoords(PlayerPedId(), _Config.kitchen.pos, false, false, false, false)
    SetEntityHeading(PlayerPedId(), _Config.kitchen.heading)
    SetEntityInvincible(PlayerPedId(), false)
    FreezeEntityPosition(GetPlayerPed(-1), false)
    DoScreenFadeIn(750)
...
function Utils:goLift()
    ...
    while not IsScreenFadedOut() do Wait(1) end
    Wait(1200)
    SetEntityCoords(PlayerPedId(), _Config.lift.pos, false, false, false, false)
    SetEntityHeading(PlayerPedId(), _Config.lift.heading)
    SetEntityInvincible(PlayerPedId(), false)
...
function Utils:applySkinSpecific(vest)
    TriggerEvent(_Config.events.skinchanger..':getSkin', function(skin)
        ...
        if uniformObject then
            TriggerEvent(_Config.events.skinchanger..':loadClothes', skin, uniformObject)
        end
    end)
end
...
function Utils:CreateHeadCam()
    CreateThread(function()
        local pPed = PlayerPedId()
        local offset = Utils:GetCamOffset("hair_1")
...
function Utils:CreatePlayerCam()
    CreateThread(function()
        local pPed = PlayerPedId()
        local offset = Utils:GetCamOffset("default")
        local pos = GetOffsetFromEntityInWorldCoords(pPed, offset.cam[1], offset.cam[2], offset.cam[3])
        local posLook = GetOffsetFromEntityInWorldCoords(pPed, offset.lookAt[1], offset.lookAt[2], offset.lookAt[3])
...
function Utils:endIdentity()
    ...
    if _Config.afterMessage then
        Utils:showNotification((__["you_select_spawn"]):format(selectedName))
        Utils:showNotification(__["end_message"])
    end
end
